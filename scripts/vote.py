...
def fetch_vote_result(
    ...
    try:
        ...
        for row in root.iter("row"):
            # ncocpgfiaoituanbr 응답에는 ABSENT_TCNT/RESULT가 없어
            # MEMBER_TCNT - VOTE_TCNT를 불참으로, PROC_RESULT_CD를 결과로 사용
            member_tcnt = _int(row, "MEMBER_TCNT")
            vote_tcnt = _int(row, "VOTE_TCNT")
            absent = max(member_tcnt - vote_tcnt, 0)
            vr = VoteResult(
                bill_id=_text(row, "BILL_ID"),
                bill_no=_text(row, "BILL_NO"),
                bill_name=_text(row, "BILL_NAME"),
                vote_date=_text(row, "PROC_DT") or _text(row, "VOTE_DATE"),
                total=member_tcnt,
                yes=_int(row, "YES_TCNT"),
                no=_int(row, "NO_TCNT"),
                abstain=_int(row, "BLANK_TCNT"),
                absent=absent,
                result=_text(row, "PROC_RESULT_CD") or _text(row, "RESULT"),
            )
            results.append(vr)
...
def fetch_member_votes(
    ...
    nojepdqqaweusdfbi 서비스는 BILL_ID(의안 고유 PRC_xxx 문자열) 필수.
    """
    if not ASSEMBLY_API_KEY:
        return []
    ...
    logger.info(f"의원별 표결 조회: bill_id={bill_id}")
...
def build_vote_metadata(
    ...
    # 의원별 표결 (선택) — BILL_ID + 매칭된 대수 사용
    if include_member_votes and vr.bill_id:
        member_votes = fetch_member_votes(vr.bill_id, assembly_age=matched_age)
        if member_votes:
            meta["member_votes"] = [
                {
                    "name": mv.member_name,
