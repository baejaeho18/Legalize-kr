...
def fetch_proposers(bill_id: str) -> list[Legislator]:
    ...
    URL/구조 변경 이력:
      - 구: /bill/coactorListPopup.do — <div id="periodDiv"> 안 <a> 텍스트
      - 신: /bill/bi/popup/billProposer.do — <ul class="member_list_img"> 안
            <li> > <a href="…/members/22nd/{ENGLISH_ID}"> > <p>한글이름</p>
    새 페이지에는 정당 정보가 없어 party 필드는 비워둔다.
    User-Agent 미설정 시 likms가 차단(`Bad Request`)하므로 명시.
    """
    url = f"https://likms.assembly.go.kr/bill/bi/popup/billProposer.do?billId={bill_id}"
    headers = {"User-Agent": _LIKMS_UA}
    legislators = []
    ...
    try:
        ...
        proposer_ul = soup.find("ul", class_="member_list_img")
        if not proposer_ul:
            logger.warning(f"발의의원 정보를 찾을 수 없음: bill_id={bill_id}")
            return []

        for li in proposer_ul.find_all("li"):
            name_el = li.find("p")  # 첫 <p>가 한글 이름
            ...
            if a_tag and a_tag.get("href"):
                # 예: https://www.assembly.go.kr/members/22nd/LEEKIHEON
