...
│   ├── by-type/
│   └── by-ministry/
├── scripts/                        # 운영 스크립트 (Python)
│   ├── main.py                     # 오케스트레이터
│   ├── config.py                   # 설정 + 분야 매핑
│   ├── fetcher.py                  # 국가법령정보 API
│   ├── assembly.py                 # 열린국회정보 API + likms 크롤러
│   ├── vote.py                     # 본회의 표결 수집
│   ├── courtcase.py                # 판례 연동
│   ├── english.py                  # 영문 법령
│   ├── crossref.py                 # 상호참조·하위법령
│   ├── converter.py                # XML → Markdown + 분야 분류
│   ├── committer.py                # Git 커밋 엔진
│   ├── rss.py                      # RSS/Atom
│   ├── search_index.py             # 검색 인덱스
│   └── adapter.py                  # legalize-dev 호환
├── .github/workflows/update.yml    # 매일 KST 06:00 자동 업데이트
├── requirements.txt
├── LICENSE
└── README.md
```

분야는 법제처 공식 분야별 분류를 참고한 휴리스틱(법령명 키워드 + 소관부처 fallback)입니다.
정확도가 중요한 사용처에서는 분류 결과를 별도로 검증해주세요.
...
| Secret 이름 | 설명 | 필수 |
|-------------|------|------|
| `LAW_API_OC` | 국가법령정보 API의 OC 값 | 필수 |
...
- 다른 나라 법령 어댑터 구현
- https://news.hada.io/topic?id=30025 : 선거 공약 이행율과의 연계
